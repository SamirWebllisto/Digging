...
export default function InvitationPower() {
  const { t } = useTranslation()

  return (
    <NavHeaderLayout className='pt-4' contentClassName='px-4' pageName='邀请算力收益' headerChildren={<span className="text-white font-normal text-base">{t('invitation.power')}</span>}>
            ...
            <div className="text-wow-100 text-xs px-5">
              <Lists
                request={async (params) => {
                  const { result } = await request.get('/user/getInfiniteStartMiningList', {
                    params,
                  })
